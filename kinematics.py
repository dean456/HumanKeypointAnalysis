...
class Kinematics:
    ...
    def getAngleNonCorView(k1,k2,maxLen): # NonCor: non coronal view = sagittal view or axial view. Angle: -90 < degree < 90
        if (maxLen > 0):
            return math.degrees(np.arccos(Kinematics.getDistance(k1,k2)/maxLen))
        else:
            return 0
...
class Plots:
    def getMyPlot(fig,title,legend,labelx,labely,subjectname,x1,y1,c1='k-',x2=[],y2=[],c2=[]):
        plt.figure(fig)
        plt.plot(x1,y1,'k-')
        ...
        plt.ylabel(labely)
        plt.grid()
        plt.savefig(subjectname+"_"+title+".png") # default png
        #plt.show()
